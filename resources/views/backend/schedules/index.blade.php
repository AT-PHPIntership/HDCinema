...
<div class="row" id="create_film">
    <div class="col col-md-12">
        <a href="{{route('admin.schedule.create')}}" class="btn btn-primary">{{ trans('lang_admin.schedule.create_schedule') }}</a>
    </div>
</div>
    ...
    <div class="col col-md-12">
        <div class="table-responsive">
            {!! Form::open(['route' => ['admin.schedule.show'],'method' => 'GET', 'id' => 'schedule-form']) !!}  
                ...
                <div id="info" class="col-md-12 no-padding">
                    <div class="col-md-3"><b>{{ trans('lang_admin.schedule.day') }}</b> {{$info['day']->name}}</div>
                    <div class="col-md-3"><b>{{ trans('lang_admin.schedule.cinema') }}</b> {{$info['cinema']->name}}</div>
                </div>
            ...
            <table id="list_schedules" class="table table-bordered table-striped" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center col-film">{{ trans('lang_admin.schedule.film') }}</th>
                        <th class="text-center col-schedule">{{ trans('lang_admin.schedule.schedule') }}</th>
                        <th class="text-center col-room">{{ trans('lang_admin.schedule.room') }}</th>
